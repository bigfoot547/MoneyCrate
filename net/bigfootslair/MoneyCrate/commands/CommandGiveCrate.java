...
public class CommandGiveCrate implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender.hasPermission("moneycrate.givecrate")) {
            if (args.length == 3) {
                Player target = Bukkit.getPlayerExact(args[0]);
                if (target == null) {
                    if (args[0].equals("*")) {
                        Bukkit.getServer().dispatchCommand(sender, "givemoneycrateall " + args[1] + " " + args[2]);
                    } else {
                ...
                sender.sendMessage(ChatColor.RED + "Usage: /givemoneycrate <player|*> <tier> <amount>");
            }
        } else {
